...
class AppointmentController extends Controller
{
    ...
    public function getAvailableTimes($doctorId, $date)
    {
        ...
        while ($start <= $end->copy()->subMinutes(30)) {
            if ($selectedDate->isSameDay($now) && $start->lessThan($now)) {
                $start->addMinutes(30);
                continue;
            }
        ...
        Log::info("message", ['bookedSlotsRaw' => $bookedSlotsRaw]);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($user->role = "admin") {return redirect()->route('admin.appointments')->with('success', 'Appointment updated successfully!');

            // return to admon acount
        }   

            // return to user acount
    ...
    public function pastAppointments()
    {
        ...
        $pastAppointments = $doctor->appointments()
            ->where('status', 'completed')
            ->orderBy('created_at', 'desc')
            ->get();

        return view('doctor.past_appointments', compact('pastAppointments'));
    }
